...
export async function handleIssueComment(
  ...
  if (decision.target === "review") {
    ...
    // skipping — the agent still resolves the real diff via `gh pr diff`. The
    // D1 claim/confirm slot resolution below is independent of that fallback —
    // it runs regardless of whether `prDetails` fetch succeeded.
    const reviewSlot = await resolveReviewSession(
      env.CONTROL_PLANE,
      headers,
      repoFullName,
