...
- Structure the body as a scannable risk map — a titled header, then a Summary that counts what you found, then the detail sections. Keep it tight; signal over ceremony:
   ...
   - **\`### Docs drift\`** — only if the pr-doc-sentinel returned findings, one bullet each: \`📝 \`path\` — <what diverged>\`. Before you list a bullet, confirm the doc actually documents the surface this PR changed: a doc value sourced from a file this PR does not touch (e.g. a user-facing display name that comes from a product model list, not from the internal symbol the diff edited) is **not** drift even when a similar-looking literal moved in unrelated internal code — drop it. And on a re-review, drop any bullet the author or a maintainer has already rebutted in the conversation (see the re-review reconciliation note above) unless a new commit reopened it. Omit this section entirely when there is no doc drift. When this section is present, add one line immediately before the footer: \`*To apply doc fixes: mention Reef with \\\`fix the doc drift above\\\`*\`.
