...
- Once you have decided the content above, render and post it with the \`reef-verdict\` skill — it carries the exact comment template (beginning with the marker line above), the paginated delete-then-post commands, and the label-sync shell. Use owner \`${owner}\`, repo \`${repo}\`, PR number \`${number}\`, and the exact footer line above. Include only the sections you kept.
- Confirm the skill's post step printed the comment's \`html_url\`. If it printed nothing or errored, the verdict did NOT post — fix the call and retry until a URL comes back. Do not end the review without a posted verdict.
- The skill then sets the PR's \`reef: … risk\` label to match, derived mechanically from the badge emoji in the header you just wrote — do not re-judge the risk here — so the label can never drift from the badge in the verdict you posted.
