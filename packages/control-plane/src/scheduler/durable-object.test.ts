...
vi.mock("../db/integration-settings", () => ({
  IntegrationSettingsStore: function () {
    return { getResolvedConfig: mockGetResolvedConfig };
  },
}));
...
describe("SchedulerDO", () => {
  ...
  describe("sandboxSettings forwarding in automation sessions", () => {
    it("passes resolved sandboxSettings into the DO init body", async () => {
      ...
      expect(initCall).toBeDefined();
      const initInput = initCall?.[0];
      const initRawBody =
        initInput instanceof Request
          ? await initInput.clone().text()
          : String(initCall?.[1]?.body ?? "");
      const initBody = JSON.parse(initRawBody);
