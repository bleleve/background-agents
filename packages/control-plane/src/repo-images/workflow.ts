...
// Surface repos whose image builds keep failing: a persistent failure streak
// means no fresh image, so every session falls back to a full cold boot.
...
export class RepoImageBuildWorkflow {
  ...
  async acceptBuildFailed(command: AcceptBuildFailedCommand): Promise<RepoImageWorkflowResult> {
    ...
    try {
      ...
      // Best-effort: warn when a repo's builds keep failing with no fresh image.
      // Never let this turn a successful markBuildFailed into a workflow error.
      try {
        const streak = await this.store.countFailuresSinceLastReady(
          failureInput.failure.buildId,
          provider
        );
        if (streak && streak.failureCount >= REPEATED_BUILD_FAILURE_ALERT_THRESHOLD) {
          logger.warn("repo_image.repeated_build_failures", {
            ...
            latest_error: failureInput.failure.errorMessage,
            request_id: ctx.request_id,
            trace_id: ctx.trace_id,
          });
        }
        ...
        logger.warn("repo_image.failure_streak_check_error", {
          build_id: failureInput.failure.buildId,
          error: errorMessage(streakError),
          request_id: ctx.request_id,
          trace_id: ctx.trace_id,
        });
      }
