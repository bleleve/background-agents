...
import { createRwxRestClient } from "./rwx-rest-client";
...
import { createRwxProvider, type RwxSandboxProvider } from "./providers/rwx-provider";
...
function createRwxProviderFromEnv(env: Env): RwxSandboxProvider {
  if (!env.RWX_ACCESS_TOKEN) {
  ...
  return createRwxProvider(client, {
    scmProvider: resolveScmProviderFromEnv(env.SCM_PROVIDER),
    codeServerPasswordSecret: env.RWX_ACCESS_TOKEN,
    orgSlug: env.RWX_ORG_SLUG,
  });
}
...
export function createSandboxProviderFromEnv(env: Env, backend: "rwx"): RwxSandboxProvider;
export function createSandboxProviderFromEnv(
  env: Env,
...
export function createSandboxProviderFromEnv(
  ...
  switch (backend) {
    ...
    case "rwx":
      return createRwxProviderFromEnv(env);
    case "modal":
