...
class TestRebuildRepoImages:
    ...
    @pytest.mark.asyncio
    async def test_uses_head_ref_regardless_of_repo_default_branch(self):
        """Should always query HEAD, ignoring enabled-repos.defaultBranch.

        ls-remote resolves HEAD to the default-branch tip, so the scheduler
        never needs the branch name even when the repo reports a non-"main"
        default branch. The build path resolves the real branch name when it
        ...
        mock_ls_remote.assert_any_call("acme", "repo", "HEAD", "")
