...
SIGNATURE_STAGING=$(printf '%s' "$TIMESTAMP" | openssl dgst -sha256 -hmac "$INTERNAL_CALLBACK_SECRET_STAGING" -hex | awk '{print $NF}')
...
curl -X PUT 'https://open-inspect-linear-bot-fountain.fountain.workers.dev/config/team-repos' \
  ...
  "bf11c370-32f6-45bc-b950-e22461d6572d": [
    ...
    { "owner": "onboardiq", "name": "background-agents", "label": "background-agents" },
...
curl -X PUT 'https://open-inspect-linear-bot-ftn.fountain.workers.dev/config/team-repos' \
  -H "Authorization: Bearer ${TIMESTAMP}.${SIGNATURE_STAGING}" \
  -H "Content-Type: application/json" \
  -d '{
  "95e25668-877e-40c7-926b-76f0192af4d5": [
    { "owner": "onboardiq", "name": "megalith", "label": "megalith" },
