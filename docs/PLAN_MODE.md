...
| ---------- | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| **Web**    | If you never touch the `Plan` toggle, the classifier decides from what you've typed so far — same read as Slack's. | Click `Plan` before submitting. The model selector swaps to the deployment's plan model. A click always wins over the classifier. | Click `Plan` to force it OFF, or just leave it untouched and write a small, well-scoped request. |
