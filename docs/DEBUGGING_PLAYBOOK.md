...
| `sandbox.ready_status_recovered` | info       | `from_status` (`spawning`\|`connecting`), `trigger` (the event type that proved liveness — `ready`, or an agent-activity event such as `step_start`/`tool_call`) | Sandbox stuck in a booting status was self-healed to `ready` (a lost WS-upgrade `ready` write); a heartbeat baseline is also seeded so the connecting-timeout watchdog stops measuring from `created_at` |
...
| `sandbox.spawn_failed`               | error | `error`                                                           | Spawn failed (permanent/unknown error) — sandbox marked `failed`, circuit breaker incremented                                                                                                                                                   |
| `sandbox.spawn_create_indeterminate` | warn  | `error`                                                           | Transient create error (gateway/timeout/abort); the sandbox may still be coming up, so it is kept `connecting` (no breaker) and adopted on a late connect — the pre-armed watchdog fails it via `connecting_timeout` only if none ever connects |
...
| `sandbox.heartbeat_stale_deferred`   | info  | `last_heartbeat_ms`, `in_flight_silence_ms`, `last_activity`, `silence_backstop_ms` | Stale-heartbeat terminal action deferred: a turn is in flight and agent activity (`last_activity`) is recent enough that `in_flight_silence_ms` is below the backstop — the agent is demonstrably alive even though the bridge heartbeat lapsed |
| `sandbox.boot_progress`              | debug | `sandbox_status`                                                  | Boot-progress ping received; heartbeat refreshed and connecting-timeout alarm re-armed                                                                                                                                                          |
| `sandbox.boot_progress_ignored`      | debug | `sandbox_status`                                                  | Boot-progress ping arrived after sandbox left the booting phase; no-op                                                                                                                                                                          |
| `sandbox.connecting_timeout`         | warn  | `elapsed_ms`, `timeout_ms`, `created_at`, `last_heartbeat`, `now` | Bridge never connected in time; `last_heartbeat` reflects the most recent boot-progress ping                                                                                                                                                    |
| `sandbox.connecting_timeout_deferred` | info  | `elapsed_ms`, `in_flight_silence_ms`, `last_activity`, `silence_backstop_ms` | Connecting-timeout terminal action deferred: a turn is in flight and the combined silence clock (`max(last_heartbeat, last_activity)`) is below the backstop — slow restore/respawn with a live agent is spared                               |
