...
- D1 migrations from `terraform/d1/migrations/` (and `fork/`) are applied automatically via
  `test/integration/apply-migrations.ts` — see [docs/MIGRATIONS.md](docs/MIGRATIONS.md)
...
- **D1 migrations**: the numeric `NNNN_` range in `terraform/d1/migrations/` is reserved for
  upstream; fork-local migrations go in `terraform/d1/migrations/fork/` with timestamp prefixes via
  `scripts/new-migration.sh`. Never hand-create a numeric migration. See
...
- [docs/MIGRATIONS.md](docs/MIGRATIONS.md) — D1 migration layout (upstream vs fork), naming,
  reconcile
